...
NOISE_CACHE_DURATION = FRAME_DURATION * 20
...
def audio_callback(outdata, frames, time_info, status):
    with brightness_lock:
        gains = current_brightness.copy()
    output = np.zeros(frames)
    # pick one random start per band
    starts = np.random.randint(0, long_noise_len - frames, size=LINE_HEIGHT)
    for i in range(LINE_HEIGHT):
        buf = noise_cache[i]
        start = starts[i]
        noise_slice = buf[start:start + frames]
        output += (1-gains[i]) * noise_slice
    outdata[:] = output.reshape(-1, 1).astype(np.float32)
