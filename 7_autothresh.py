...
AUTO_THRESH_MARGIN = 0.40   # 20% inside min/max (change to 0.10, 0.25, etc.)
MIN_THRESH_GAP     = 0.05   # ensure UPPER - LOWER is at least 0.05
...
def auto_set_thresholds_from_capture(cx, cy):
    """
    Look at ALL pixels in the capture area, compute darkness in [0,1],
    and set LOWER/UPPER to sit AUTO_THRESH_MARGIN inside the min/max.
    Guarantees a minimum gap (MIN_THRESH_GAP) and clamps to [0,1].
    """
    global LOWER_THRESH, UPPER_THRESH

    arr_rgb = capture_mouse_region(cx, cy).astype(np.float32)  # HxWx3
    # Darkness in [0,1], consistent with get_vertical_line():
    # 1 - (R+G+B)/(255*3)  ==  1 - mean(rgb)/255
    darkness = 1.0 - np.clip(arr_rgb.mean(axis=2) / 255.0, 0.0, 1.0)

    d_min = float(darkness.min())
    d_max = float(darkness.max())
    span  = d_max - d_min

    if span <= 1e-6:
        # Essentially uniform—center and enforce minimum gap
        center = np.clip(d_min, 0.0, 1.0)
        half   = MIN_THRESH_GAP * 0.5
        LOWER_THRESH = max(0.0, center - half)
        UPPER_THRESH = min(1.0, center + half)
    else:
        margin = span * AUTO_THRESH_MARGIN
        lo = d_min + margin
        hi = d_max - margin

        # Enforce min gap
        if (hi - lo) < MIN_THRESH_GAP:
            center = 0.5 * (lo + hi)
            half   = MIN_THRESH_GAP * 0.5
            lo = center - half
            hi = center + half

        # Clamp to [0,1] and also ensure order
    ...
    print(f"[AutoThresh] LOWER={LOWER_THRESH:.3f}  UPPER={UPPER_THRESH:.3f}  "
          f"(min={d_min:.3f}, max={d_max:.3f}, span={span:.3f})")
...
def draw_mouse_region_view(surface, x, y):
    ...
    h, w = arr_rgb.shape[:2]
    surf = pygame.image.frombuffer(arr_rgb.tobytes(), (w, h), "RGB").copy()
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # --- NEW: Auto-threshold from capture area (keys 0 or 5) ---
                elif event.key in (pygame.K_0, pygame.K_5):
                    # use whichever coordinates you're tracking (follow or frozen)
                    auto_set_thresholds_from_capture(TRACK_X, TRACK_Y)
