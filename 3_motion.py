...
ONLY_SHOW_MOVEMENT = False   # ⬅️ turn on/off here
...
def apply_threshold_scale(x, lo=LOWER_THRESH, hi=UPPER_THRESH, in_max=INPUT_RANGE_MAX):
    ...
    if ONLY_SHOW_MOVEMENT:
        if _prev_scaled is not None and _prev_scaled.shape == y_scaled.shape:
            ...
            y_out = y_scaled.copy()
        # Store the unmasked scaled values for next-frame comparison
        _prev_scaled = y_scaled
        return y_out
    else:
        # Even when disabled, keep previous up to date
        _prev_scaled = y_scaled
        return y_scaled
