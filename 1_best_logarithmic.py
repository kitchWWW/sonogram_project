...
try:
    while running:
        ...
        with brightness_lock:
            current_brightness = brightness.astype(np.float32, copy=False)

        # Update visual history
        update_waterfall_history(wf_columns, brightness, WATERFALL_COLS)
        ...
        avg_dark = float(np.mean(brightness)) if brightness.size else 0.0
